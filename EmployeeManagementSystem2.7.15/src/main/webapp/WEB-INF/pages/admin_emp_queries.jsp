...
<h1 style="color: green; text-align: center;">Employee Queries</h1>
	<hr>
	<h3 style="color:green;text-align:center;"><blink>${message }</blink></h3>
	<c:choose>
		<c:when test="${!empty emp_queries }">
			<div class="container" id="div1">
				<table class="table table-striped">
                    ...
                    <% int i=1; %>
					<c:forEach var="query" items="${emp_queries}">
						<tr>
						    <td><%=i++ %></td>
							<td>${query.empName }</td>
							...
							<td>${query.message}</td>
							<td>${query.status }</td>
							<td><a href="send_response_to_emp_query?empId=${query.empId}&helpId=${query.msgId}">Response</a>
							</tr>
					</c:forEach>
				</table>
			</div>
